/*
 * psdgen_ps_simple.jsx — PSD 排版生成（简易版，不含文本图层更新）
 *
 * 功能：
 *   1. 选择排版文本文件（.txt）
...
function showDialog(state) {
    var win = new Window("dialog", "PSD 排版生成");
...
function main() {
    ...
    // ---- 调用 psdgen.exe 生成 PSD（隐藏窗口立即启动），并等待完成 ----
    var outDir = new Folder(txtFile.parent.fsName + "/output");
    var gen = runGenerator(exe, txtFile, cfgPath, null);
